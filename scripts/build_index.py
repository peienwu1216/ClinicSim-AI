...
def build_index():
    ...
    # 4. 初始化 Embedding 模型 (使用 NPU 優化)
    print(f"正在初始化 Embedding 模型: {EMBEDDING_MODEL} (將嘗試使用 NPU)...")
    
    # 指定模型在 IPU (AMD NPU 的 ONNX Runtime 代稱) 上運行
    # 如果遇到問題，可以改為 'cuda' (NVIDIA GPU) 或 'cpu'
    # 由於環境中的 PyTorch 不支援 'ipu'，我們退回至 'cpu'
    model_kwargs = {'device': 'cpu'}
    # 根據您的建議，設定 normalize_embeddings 為 False
    ...
    # 5. 建立 FAISS 索引並儲存
    print("正在將知識片段轉換為向量並建立 FAISS 索引 (此步驟將利用 NPU)...")
