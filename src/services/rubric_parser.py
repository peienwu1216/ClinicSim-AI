...
class RubricParser:
    ...
    def _extract_keywords(self, description: str) -> List[str]:
        """從描述中提取關鍵字 - 大幅改進的邏輯"""
        # 基於描述內容提取相關關鍵字 - 更全面的關鍵字庫
        keyword_mapping = {
            "intro": ["自我介紹", "我是", "醫生", "同意", "隱私", "舒服", "可以嗎", "確認", "你好", "您好", "請問", "開始"],
            "opqrst": ["什麼時候", "發作", "開始", "突然", "誘發", "什麼會讓", "活動", "深呼吸", 
                      "什麼樣", "壓", "悶", "刺痛", "性質", "延伸", "放射", "肩膀", "下巴",
                      "幾分", "多痛", "嚴重", "程度", "多久", "持續", "間歇", "時間", "哪裡", "怎麼", "如何"],
            "associated_symptoms": ["喘", "呼吸困難", "冷汗", "盜汗", "噁心", "想吐", "嘔吐", "咳嗽", "發燒", "呼吸", "出汗"],
            "ice": ["想法", "擔心", "期待", "希望", "害怕", "擔憂", "你覺得", "你認為", "擔心", "害怕"],
            "acs": ["胸痛", "壓迫", "悶", "放射", "肩膀", "下巴", "菸", "抽菸", "高血壓", "糖尿病", 
                   "家族史", "心臟病", "膽固醇", "風險因子", "既往史", "心臟", "心", "痛"],
            "aortic_dissection": ["撕裂", "撕開", "高血壓", "馬凡", "馬凡氏症", "主動脈", "撕裂痛"],
            "pe": ["久坐", "長途", "飛機", "開車", "下肢", "腫", "咳血", "肺栓塞", "肺", "栓塞"],
            "pericarditis": ["體位", "姿勢", "坐起來", "往前傾", "心包炎", "姿勢", "體位"],
            "gerd": ["進食", "吃飯", "火燒心", "胃食道", "逆流", "胃", "進食", "飯後"],
            "costochondritis": ["胸壁", "壓痛", "肌肉", "骨骼", "按壓", "胸壁", "肌肉"],
            "ecg": ["心電圖", "ECG", "12導程", "12導", "立刻", "馬上", "立即", "10分", "十分", "心電圖", "檢查"],
            "troponin": ["troponin", "心肌鈣蛋白", "心肌酵素", "抽血", "檢驗", "血液", "心肌", "酵素"],
            "cxr_ct": ["胸部X光", "CXR", "CT", "電腦斷層", "影像", "X光", "胸部", "影像"],
            "list_3": ["鑑別", "診斷", "可能", "考慮", "懷疑", "排除", "診斷", "可能"],
            "acs_first": ["急性冠心症", "ACS", "AMI", "心肌梗塞", "首要", "優先", "急性", "心肌"],
            "non_cardiac": ["胃食道", "肌肉", "骨骼", "非心因性", "非心臟", "胃", "肌肉"],
            "tests": ["檢查", "檢驗", "ECG", "心電圖", "抽血", "X光", "檢查", "檢驗"],
            "empathy": ["理解", "同理", "安撫", "安慰", "別擔心", "會好的", "理解", "安慰"],
            "summary": ["總結", "回顧", "整理", "歸納", "剛才", "總結", "回顧"],
            "safety": ["安全", "叮嚀", "注意", "小心", "下一步", "安排", "安全", "注意"]
        }
        ...
        # 1. 精確匹配：檢查描述中是否包含特定詞彙
        for key, words in keyword_mapping.items():
            # 檢查描述中是否包含該類別的關鍵詞
            if any(word in description_lower for word in words):
                # 添加更多相關關鍵字
                keywords.extend(words[:5])  # 增加到5個關鍵字
        
        # 2. 通用關鍵字：添加常見的問診詞彙
        general_keywords = ["什麼", "哪裡", "怎麼", "為什麼", "如何", "什麼時候", "多久", "多", "很", "痛", "症狀", "感覺", "檢查", "診斷", "治療", "病人", "醫生"]
        keywords.extend(general_keywords)
        
        # 3. 如果沒有匹配到特定關鍵字，使用通用關鍵字
        if not keywords:
            keywords = self._extract_generic_keywords(description)
        
        # 4. 去重並限制總數
        unique_keywords = list(set(keywords))
        return unique_keywords[:15]  # 增加到15個關鍵字
